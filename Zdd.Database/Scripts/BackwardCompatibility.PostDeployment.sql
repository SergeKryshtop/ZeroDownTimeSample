...
/************************************************************************
* Update Associateadentity type of Comlaint to NULL if complaint is not Appeal
* Update Complaint Subtype from 'Not a QOC' to NULL in all complaints
* BEGIN
**************************************************************************/
DECLARE @BreakingChangesVersion_2 nvarchar(10);
SET  @BreakingChangesVersion_2 = '08.03.0004'

IF NOT EXISTS(SELECT NULL FROM dbo.DatabaseVersion WHERE VersionNumber >= @BreakingChangesVersion_2)
BEGIN
	WHILE EXISTS (SELECT TOP (1) 1 FROM dbo.Complaint WHERE ComplaintTypeId != 1 AND AssociatedEntityTypeId IS NOT NULL)
	BEGIN
	  UPDATE TOP (1000) dbo.Complaint
	  SET AssociatedEntityTypeId = NULL
	  WHERE ComplaintTypeId != 1 
	  AND AssociatedEntityTypeId IS NOT NULL

	  IF @@ROWCOUNT = 0
	  BREAK
	END

	WHILE EXISTS (SELECT TOP (1) 1 FROM dbo.Complaint WHERE ComplaintSubTypeId = 15)
	BEGIN
	  UPDATE TOP (1000) dbo.Complaint
	  SET ComplaintSubTypeId = NULL
	  WHERE ComplaintSubTypeId = 15
...
/************************************************************************
* Update AnGComplaintId to ExtensionId for all existing Extensions without AnGComplaintId
* BEGIN
**************************************************************************/
DECLARE @BreakingChangesVersion_4 nvarchar(10);
SET  @BreakingChangesVersion_4 = '09.02.0005'

IF NOT EXISTS(SELECT NULL FROM dbo.DatabaseVersion WHERE VersionNumber >= @BreakingChangesVersion_4)
BEGIN
	WHILE EXISTS (SELECT TOP (1) 1 FROM dbo.Extension WHERE AnGComplaintId IS NULL)
	BEGIN
	  UPDATE TOP (1000) dbo.Extension
	  SET AnGComplaintId = ExtensionId
	  WHERE AnGComplaintId IS NULL
...
/************************************************************************
* Update AnGComplaintId to AuthorizedRepresentativeId for all existing AuthorizedRepresentatives without AnGComplaintId
* BEGIN
**************************************************************************/

IF NOT EXISTS(SELECT NULL FROM dbo.DatabaseVersion WHERE VersionNumber >= @BreakingChangesVersion_4)
BEGIN
	WHILE EXISTS (SELECT TOP (1) 1 FROM dbo.AuthorizedRepresentative WHERE AnGComplaintId IS NULL)
	BEGIN
	  UPDATE TOP (1000) dbo.AuthorizedRepresentative
	  SET AnGComplaintId = AuthorizedRepresentativeId
	  WHERE AnGComplaintId IS NULL
